...
class GeneralVisualizer:
    def __init__(self, cube:mCube): 
        self.cube = cube
        self.colors = cube.get()
    def visualize(self, figsize=(7,7)):
        colors = self.colors
        if len(colors) != 54:
            raise ValueError("colors must be exactly 54 characters")

        # Map color letters to visible matplotlib colors
        color_map = {
            'W': 'white', 'Y': 'yellow', 'R': 'red',
            'O': 'orange', 'G': 'green', 'B': 'blue'
        }

        # Net layout: faces arranged in the standard Rubik’s layout
        face_positions = {
            'U': (0, 1),
            'L': (1, 0),
            'F': (1, 1),
        ...
        for i, face in enumerate("ULFRBD"):
            fr, fc = face_positions[face]
            face_colors = colors[i*9:(i+1)*9]

            for j, c in enumerate(face_colors):
                row = j // 3
                col = j % 3
                x = fc * 3 + col
                y = 8 - (fr * 3 + row)
                ...
                rect = patches.Rectangle(
                    (x, y), 1, 1,
                    facecolor=color_map.get(c, 'lightgray'),
                    edgecolor='black',
                    linewidth=1
                )
                ax.add_patch(rect)
...
class Visualizer:
    def __init__(self, cube:CryptoCube):
        ...
        self.mode = cube.mode
    def reverse_moves_R(self, moves: list):
            ...
            return inverse
    def reverse_moves_M(self, moves: list):
            '''For  rubik cube module only'''
            try:
                moves = list(moves)
            except TypeError:
            ...
            for move in reversed(moves):
                if move == "":
                    continue  # skip empty moves
                if move.endswith("'"):  # inverse move (like Ri → R)
                    inverse.append(move[:-1])  
                else:  # normal move (like R → Ri)
                    inverse.append(move + "'")  
            return inverse

    def display_flat_cube(self,colors: str,
                            labels: str,
        ...
        """
        labels = self.reformat_cube(labels)
        # --- validation
        if len(colors) != 54:
    ...
    def reformat_cube(self,cube_str):
        """
    ...
    def visualize_encrypt(self, A_moves, plaintext, ciphertext):
        ...
        self.display_flat_cube(key_cube.get(), permutation_cube.flat_str())
        ...
        self.display_flat_cube(IV_cube.get(), permutation_cube.flat_str())
    ...
    def visualize_decrypt(self, A_moves, plaintext, ciphertext):
        ...
        #         permutation.insert(i, '_')
        # except:
        #     raise ValueError("ciphertext: {ciphertext} is in the wrong format or mismatched modes")
        ...
        #------------------------------------------------------------------
         # come up with the move sequences first
        
        key_cube.rotate(A_moves)
        partial_cube = mCube(3, str(key_cube.get()))
        solver = BasicSolver(partial_cube)
        solver.solve()
        
        Bmoves_i = [str(i).replace("'", "i") for i in partial_cube.history()] #inverse moves B
        Bmoves_i = " ".join(self.reverse_moves_R(Bmoves_i)) #inverse moves B
        Amoves_i = [str(i).replace("'", "i") for i in A_moves.split(" ")]
        Amoves_i = " ".join(self.reverse_moves_R(Amoves_i))

        key_cube =  mCube(3, str(self.key_cube.get())) # resets keycube
        ...
        #Second Display Permutation and Key Cube After inverse B_moves 
        permutation_cube.sequence(Bmoves_i)
        key_cube.rotate(A_moves)
...
""")
    ...
    # visualizer.visualize_decrypt(A_moves, plaintext, ciphertext)

    visualizer = GeneralVisualizer(key_cube)
